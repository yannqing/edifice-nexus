...
{	
	...
	public function edifice_sso()
	{
		$admin = Db::name('Admin')->where(['id' => $this->uid, 'delete_time' => 0])->find();
		if (empty($admin) || (int)$admin['status'] !== 1) {
		...
		$token = JWT::encode([
			...
			'oaAdminId' => (int)$admin['id'],
			'oaUserid' => (string)$admin['userid'],
