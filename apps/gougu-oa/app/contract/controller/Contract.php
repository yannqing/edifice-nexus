    ...
    */
    public function view($id)
    {
		...
		if (!empty($detail)) {
			...
			$engineeringProject = ['exists' => false];
			$engineeringProjectTypes = [];
			if ((int)$detail['check_status'] === 2 && (int)$detail['stop_time'] === 0 && (int)$detail['void_time'] === 0) {
				$statusResult = EdificeSync::getContractProjectStatus((int)$detail['id']);
				if ($statusResult['ok'] && is_array($statusResult['data'])) {
					$engineeringProject = $statusResult['data'];
				}
				if (empty($engineeringProject['exists'])) {
					$typeResult = EdificeSync::getProjectTypes();
					if ($typeResult['ok'] && is_array($typeResult['data'])) {
						$engineeringProjectTypes = $typeResult['data'];
					}
				}
			}
			View::assign('detail', $detail);
	...
	/**
	 * 从已审批 OA 合同创建 Edifice 工程项目
	 */
	public function create_engineering_project()
	{
		$param = get_params();
		$id = isset($param['id']) ? (int)$param['id'] : 0;
		$projectTypeId = isset($param['project_type_id']) ? (string)$param['project_type_id'] : '';
		if (!request()->isPost() || $id <= 0 || $projectTypeId === '') {
			return to_assign(1, '合同和项目类型不能为空');
		}

		$contract = $this->model->getById($id);
		if (empty($contract) || (int)$contract['check_status'] !== 2) {
			return to_assign(1, '仅审批通过的合同可以创建工程项目');
		}
		if ((int)$contract['stop_time'] > 0 || (int)$contract['void_time'] > 0) {
			return to_assign(1, '已中止或已作废的合同不能创建工程项目');
		}
		if (!$this->canCreateEngineeringProject($contract)) {
			return to_assign(1, '您没有权限为该合同创建工程项目');
		}

		$host = request()->domain();
		$payload = [
			'projectTypeId' => $projectTypeId,
			'contractType' => isset($param['contract_type']) ? (int)$param['contract_type'] : 0,
			'baseAmount' => isset($param['base_amount']) ? $param['base_amount'] : null,
			'benefitAmount' => isset($param['benefit_amount']) ? $param['benefit_amount'] : null,
			'contractUrl' => $host . '/contract/contract/view?id=' . $id,
		];
		$result = EdificeSync::createContractProject($id, $payload);
		if (!$result['ok']) {
			return to_assign(1, $result['message']);
		}
		return to_assign(0, '工程项目创建成功', $result['data']);
	}

	private function canCreateEngineeringProject(array $contract): bool
	{
		if (isAuth($this->uid, 'contract_admin', 'conf_1')) {
			return true;
		}
		$directUserIds = [
			(int)$contract['admin_id'],
			(int)$contract['prepared_uid'],
			(int)$contract['sign_uid'],
			(int)$contract['keeper_uid'],
		];
		if (in_array((int)$this->uid, $directUserIds, true)) {
			return true;
		}
