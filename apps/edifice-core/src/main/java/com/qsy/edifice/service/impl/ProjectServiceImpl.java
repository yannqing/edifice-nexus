...
public class ProjectServiceImpl extends ServiceImpl<ProjectMapper, Project> implements ProjectService  {
    ...
    public boolean createProject(ProjectCreateDto dto, Long currentUserId) {
        ...
        //在用currentUserId查询SysUserRole表，查询role，再通过查询sysRolePermission，判断权限
        ...
        if(dto.getRoles() != null && !dto.getRoles().isEmpty()){
            saveRoles(projectId,dto.getRoles(),currentUserId);
        }
    ...
    private void validateInput(ProjectCreateDto dto, Long currentUserId) {
        if (dto == null) {
            throw new BusinessException(ErrorType.PROJECT_NOT_EXISTS);
        }
    ...
    private void saveContracts(Long projectId, List<ContractDTO> contracts) {
        ...
        List<Contract> contractList = contracts.stream().map(dto -> {
            ...
            if (dto.getContractAmount() == null || dto.getContractAmount() <= 0) {
                throw new BusinessException(ErrorType.CONTRACT_FAILED);
            }
    ...
    private void saveStages(Long projectId, String stages) {
        ...
        // 清洗输入：只取第一个非空字符（兼容 "A"、" A "、"A,B" 等，但按你的需求其实只传单个字母）
        String stageCode = stages.trim().toUpperCase();
        if (stageCode.isEmpty()) {
            throw new BusinessException(ErrorType.CONTRACT_FAILED);
        }

        // 只取第一个字符（防止传 "AB" 这种异常值）
        char code = stageCode.charAt(0);

        List<String> stageNameList;
        switch (code) {
            case 'A':
                stageNameList = Arrays.asList("立项");
                break;
            case 'B':
                stageNameList = Arrays.asList("立项", "开发");
                break;
            case 'C':
                stageNameList = Arrays.asList("立项", "开发", "测试");
                break;
            case 'D':
                stageNameList = Arrays.asList("立项", "开发", "测试", "上线");
                break;
            case 'E':
                stageNameList = Arrays.asList("立项", "开发", "测试", "上线", "运维");
                break;
            default:
                throw new BusinessException("不支持的阶段类型: " + code);
        }

        // 构造 ProjectStage 对象列表
        ...
        if (!stageList.isEmpty()) {
            projectStageService.saveBatch(stageList);
        }
    }
    ...
    private void saveRoles(Long projectId, List<SysUserRoleDto> roles,Long currentUserId) {
        ...
        List<SysUserRole> memberList = roles.stream().map(dto -> {
        ...
        sysUserRoleService.saveBatch(memberList);
    }
