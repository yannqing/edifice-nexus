...
注意，直接在 controller 方法中的入参来获取用户的 token，从 token 来解析出登录用户的 user_id
...
be committed - 已经提交的，其余是未提交的



2. git add .

提交修改的代码到本地暂存区（确保没有敏感文件）
...
- 必须是英文
- 前面有前缀（feat, fix, update, add 等）

格式： feat: add a lot of function about user table
...
(首次执行有一个关联操作，把本地 dev 与远程 dev 做一次关联，报错后，错误给AI，执行 AI 给的命令即可)
...
3. 基于目前最新的本地 dev 创建一个新的功能分支

git checkout -b feature/project-xxx-20260312

4. 推送分支到远程

git push --set-upstream origin feature/project-xxx-20260312

5. 编写代码（每写完并测试完一个接口后，执行一次AC操作 - add 和 commit）
6. git add .
7. git commit -m ""
8. 编写代码
9. AC
